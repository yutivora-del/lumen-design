...
function ToastDemo() {
  ...
  return (
    <>
      ...
      {count > 0 && <Toast key={count} message="Saved to your drafts." />}
    </>
  );
}
...
const defaultCode = `const [count, setCount] = useState(0);
...
const props: { name: string; type: string; default: string; description: string }[] = [
  { name: 'message', type: 'string', default: '—', description: 'Text to announce' },
  { name: 'duration', type: 'number', default: '3000', description: 'Milliseconds before auto-dismiss' },
  { name: 'variant', type: "'default' | 'error'", default: "'default'", description: 'Top bar colour' },
];

// ─── Component source (kept in sync with src/components/toast.tsx) ───────────────
...
export default function ToastPage() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(toastSource);
    setCopied(true);
    ...
    <>
      ...
      <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '0 32px' }}>
        ...
        <section style={{ paddingTop: '96px' }}>
          ...
          >
            A brief word, then gone. Appears in the corner and dismisses itself.
          </p>
        </section>
        ...
        <section style={{ paddingTop: '96px' }}>
          ...
          >
            Paste the source into components/toast.tsx. Announced to screen readers via aria-live — no dependencies required.
          </p>
          <div
            style={{
              position: 'relative',
