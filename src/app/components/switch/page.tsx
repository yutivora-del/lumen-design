...
function LabelDemo() {
  const [on, setOn] = useState(true);
  return <Switch checked={on} onChange={setOn} label="Reduce motion" />;
}

// ─── Example code strings ──────────────────────────────────────────────────────

const offCode = `<Switch checked={false} onChange={setOn} />`;

const onCode = `<Switch checked onChange={setOn} />`;

const labelCode = `<Switch checked={on} onChange={setOn} label="Reduce motion" />`;

// ─── Props ─────────────────────────────────────────────────────────────────────

const props: { name: string; type: string; default: string; description: string }[] = [
  { name: 'checked', type: 'boolean', default: 'false', description: 'On or off' },
  { name: 'onChange', type: '(checked: boolean) => void', default: '—', description: 'Toggle handler' },
  { name: 'label', type: 'string', default: '—', description: 'Label beside the track' },
  { name: 'disabled', type: 'boolean', default: 'false', description: 'Disables interaction' },
];

// ─── Component source (kept in sync with src/components/switch.tsx) ──────────────

const switchSource = `'use client';
...
export default function SwitchPage() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(switchSource);
    setCopied(true);
    ...
    <>
      ...
      <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '0 32px' }}>
        ...
        <section style={{ paddingTop: '96px' }}>
          ...
          >
            A toggle, on or off. The knob inverts as the track fills with ink.
          </p>
        </section>

        {/* ─── Examples ───────────────────────────────────────────────────── */}
        <section style={{ paddingTop: '96px', display: 'flex', flexDirection: 'column', gap: '40px' }}>
          <ComponentExample title="Off" code={offCode} preview={<OffDemo />} />
          <ComponentExample title="On" code={onCode} preview={<OnDemo />} />
        ...
        <section style={{ paddingTop: '96px' }}>
          ...
          >
            Paste the source into components/switch.tsx. No dependencies required.
          </p>
          <div
            style={{
              position: 'relative',
