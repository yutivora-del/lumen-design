...
const variantsCode = `<Badge>Draft</Badge>
<Badge variant="solid">Published</Badge>`;

const contextCode = `<span>Components</span>
<Badge variant="solid">v0.2</Badge>`;

// ─── Props ─────────────────────────────────────────────────────────────────────

const props: { name: string; type: string; default: string; description: string }[] = [
  { name: 'variant', type: "'default' | 'solid'", default: "'default'", description: 'Outlined or filled' },
  { name: 'children', type: 'ReactNode', default: '—', description: 'Badge text' },
];

// ─── Component source (kept in sync with src/components/badge.tsx) ───────────────

const badgeSource = `import type { CSSProperties, ReactNode } from 'react';
...
export default function BadgePage() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(badgeSource);
    setCopied(true);
          ...
          <ComponentExample
            ...
            preview={
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 540, fontSize: '15px', color: 'var(--color-text)' }}>
                  Components
                </span>
                <Badge variant="solid">v0.2</Badge>
              </div>
