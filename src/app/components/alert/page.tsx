...
const variantsCode = `<Alert variant="info" title="Saved as a draft." />
<Alert variant="warning" title="Unsaved changes." />
<Alert variant="error" title="Could not save." />`;

const titleCode = `<Alert variant="info" title="Heads up.">
...
const props: { name: string; type: string; default: string; description: string }[] = [
  { name: 'variant', type: "'info' | 'warning' | 'error'", default: "'info'", description: 'Indicator tone' },
  { name: 'title', type: 'string', default: '—', description: 'Headline message' },
  { name: 'children', type: 'ReactNode', default: '—', description: 'Optional detail line' },
];

// ─── Component source (kept in sync with src/components/alert.tsx) ───────────────

const alertSource = `import type { ReactNode } from 'react';
...
export default function AlertPage() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(alertSource);
    setCopied(true);
    ...
    <>
      ...
      <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '0 32px' }}>
        ...
        <section style={{ paddingTop: '96px' }}>
          ...
          >
            Inline messages. A hairline border and a 2px bar — the only place error red appears.
          </p>
        </section>
          ...
          <ComponentExample
            ...
            preview={
              <div style={{ width: '320px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <Alert variant="info" title="Saved as a draft." />
                <Alert variant="warning" title="Unsaved changes." />
                <Alert variant="error" title="Could not save." />
              </div>
            }
        ...
        <section style={{ paddingTop: '96px' }}>
          ...
          >
            Paste the source into components/alert.tsx. No dependencies required.
          </p>
          <div
            style={{
              position: 'relative',
