...
const defaultCode = `<Card>
  <p>A surface with a hairline border and a quiet corner radius.</p>
</Card>`;

const contentCode = `<Card>
  <p style={{ fontWeight: 540 }}>Dashboard</p>
  <p style={{ color: 'var(--color-text-muted)' }}>Four active projects.</p>
...
const props: { name: string; type: string; default: string; description: string }[] = [
  ...
  { name: 'className', type: 'string', default: '—', description: 'Optional class hook' },
];

// ─── Component source (kept in sync with src/components/card.tsx) ────────────────

const cardSource = `import type { ReactNode } from 'react';
