...
const defaultCode = `<Slider value={value} onChange={setValue} />`;

const valueCode = `<Slider min={0} max={100} step={1} value={value} onChange={setValue} />
<span>{value}</span>`;
...
// ─── Component source (kept in sync with src/components/slider.tsx) ──────────────

const sliderSource = `'use client';
...
export default function SliderPage() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(sliderSource);
    setCopied(true);
    ...
    <>
      ...
      <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '0 32px' }}>
        ...
        <section style={{ paddingTop: '96px', display: 'flex', flexDirection: 'column', gap: '40px' }}>
          ...
          <ComponentExample title="With value display" code={valueCode} preview={<ValueDemo />} />
        </section>
